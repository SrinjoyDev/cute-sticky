...
export function parseLine(line: string): Block {
  ...
  if (line.startsWith('- ') || line.startsWith('* ')) return { type: 'bullet', text: line.slice(2), done: false };
...
const marker = (b: Block) => (b.type === 'bullet' ? '• ' : b.type === 'check' ? (b.done ? '☑ ' : '☐ ') : '');
...
export function applyShortcut(b: Block): Block | null {
  if (b.type === 'p' && /^(- |\* )/.test(b.text)) return { type: 'bullet', text: b.text.slice(2), done: false };
  if (b.type !== 'check') {
    const m = /^\[( |x|X)?\] /.exec(b.text);
    if (m) return { type: 'check', text: b.text.slice(m[0].length), done: (m[1] ?? '').toLowerCase() === 'x' };
  }
  return null;
}
...
export function backspaceAtStart(blocks: Block[], i: number): { blocks: Block[]; focus: number; caret: number } {
  const cur = blocks[i];
  const out = blocks.slice();
  if (cur.type !== 'p') {
...
describe('pile layout', () => {
  ...
  it('stacked position centres a card on the visible pile', () => {
    expect(stackedTop({ step: 38, visibleH: 264, totalH: 342, push: 78, width: 192 })).toBe(264 / 2 - 55);
  });
});
```
